...
# 修改者为：decli@qq.com
# v1.2，changeLog�?
# +: 文件日期/时间/颜色显示、多线程支持、主页跳�?
# -: 解决不同浏览器下上传文件名乱码问题：仅IE，其它浏览器暂时没处理�?
# -: �?��路径显示的bug，主要是 cgi.escape() 转义问题
# ?: notepad++ 下直接编译的server路径问题

"""
    �?��：这是一�?python 写的轻量级的文件共享服务器（基于内置的SimpleHTTPServer模块），
    支持文件上传下载，只要你安装了python（建议版�?.6~2.7，不支持3.x），
    然后去到想要共享的目录下，执行：
        python SimpleHTTPServerWithUpload.py 1234        
    其中1234为你指定的端口号，如不写，默认为 8080
    然后访问 http://localhost:1234 即可，localhost 或�? 1234 请酌情替换�?
"""
...
class SimpleHTTPRequestHandler(BaseHTTPServer.BaseHTTPRequestHandler):
    ...
    def deal_post_data(self):
        plisttexts = self.headers.plisttext.split("=")
        boundary = ""
        if plisttexts != None and len(plisttexts) > 1:
            boundary = plisttexts[1]
        ...
        try:
            ...
            return (False, "文件名请不要用中文，或�?使用IE上传中文名的文件�?)
    ...
    def send_head(self):
        ...
        """
        f = None
        typeAndArgs = self.path.split("/")
        num = len(typeAndArgs)
        if typeAndArgs != None and num > 1:
            args = typeAndArgs[num - 1].split("?")
            if args != None and len(args) > 1:
                print 'agrs is', args[1]
                record = self.handler.handle_get_record(args[0], args[1])
                f = StringIO()
                print 'Data', record
                f.write(record)
                length = f.tell()
        ...
        path = self.translate_path('/doc' + self.path)
    ...
    def list_directory(self, path):
        ...
        for name in list:
            ...
            if os.path.isdir(fullname):
                colorName = '<span style="background-color: #CEFFCE;">' + \
                    name + '/</span>'
                displayname = name
                linkname = name + "/"
            if os.path.islink(fullname):
                colorName = '<span style="background-color: #FFBFFF;">' + \
                    name + '@</span>'
                displayname = name
...
class Handler():
    ...
    def handle_upload(self, fn):
        print 'Handler.hanle_upload filename', fn
